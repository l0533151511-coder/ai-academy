...
- **אסור** גרש-כפול escaped `\"` בתוך ערך attribute של JSX (למשל `why="...מע\"מ..."`) — ערכי attribute אינם מחרוזות JS; ה-`"` מסיים את הערך וה-build נשבר. לראשי-תיבות עבריים להשתמש בתו גרשיים `״` (U+05F4) או לנסח מחדש (`לו״ז`→`לוח זמנים`). אותו כלל לגבי `'` בתוך attribute עם גרש בודד.
